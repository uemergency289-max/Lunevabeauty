...
export default function ProductForm() {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit, onInvalid)} className="max-w-xl space-y-5">
      {errorBox.length > 0 && (
        <div className="p-4 bg-blush-soft border border-blush rounded-sm">
          <p className="font-body text-[13px] font-semibold text-charcoal mb-1">Please fix:</p>
          <ul className="list-disc list-inside">
            {errorBox.map((m, i) => <li key={i} className="font-body text-[12.5px] text-charcoal">{m}</li>)}
          </ul>
        </div>
      )}
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <label className="w-20 h-20 border border-dashed border-line flex items-center justify-center cursor-pointer text-[11px] text-charcoal-soft">
            {uploading ? "..." : "+ Add"}
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input {...register("sku")} className="input" placeholder="e.g. LB-001" />
        </div>
        <div>
      ...
      <style jsx global>{`
        .input { width: 100%; padding: 10px 14px; font-size: 14px; background: #fff; border: 1px solid #EAE0D5; border-radius: 2px; color: #2B2B2B; }
        .input:focus { outline: none; border-color: #C9A66B; }
      `}</style>
    </form>
  );
      }
